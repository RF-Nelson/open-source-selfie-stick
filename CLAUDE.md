...
**Status (2026-09-17):** the default layered Bluetooth + Wi-Fi transport was previously verified on physical devices. Wi-Fi Aware now preserves the selected endpoint, hands service ownership from pairing to connection, and tracks connection cancellation/readiness. Role-specific permission setup, save recovery, adaptive layouts, privacy/help, and App Store archive/export tooling are implemented. A review pass (719c0e9) added remote auto-reconnect after backgrounding, silent video when Microphone is denied, and quieter cancels. TestFlight internal build `202609170310` carries all of this. None of it has run on hardware yet: these changes still require a fresh physical-device acceptance run; SDK builds and core tests (78 passing) do not establish radio/UI behavior. The run order is at the top of `docs/TODO.md`. See `docs/APP_STORE_RELEASE.md`, `docs/TODO.md`, and `docs/TRANSPORT.md`.
...
- Hardware debugging: **Debug builds only** write `Documents/transport.log` (`Trace.log` compiles to nothing in Release, so TestFlight builds leave no log — install a Debug build with `Tools/device-debug.sh` over a cable to investigate); pull it with `xcrun devicectl device copy from ... --source Documents` (copy the **directory**, not the single file). Both phones can be USB-cabled for reliable install/log-pull; `idevicecrashreport -u <udid> -k <dir>` pulls crash `.ips` files. `idevicesyslog` is unreliable here.
