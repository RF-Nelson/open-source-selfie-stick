...
/// Fixed Core Bluetooth identifiers for Shot Caller. The camera side advertises `serviceUUID` and
...
public final class BluetoothTransport: NSObject, PeerTransport, @unchecked Sendable {
    ...
    private var activeFileSendName: String?
    ...
    public init(displayName: String) {
        ...
        inboxDirectory = FileManager.default.temporaryDirectory.appendingPathComponent("ShotCallerBLEInbox", isDirectory: true)
    ...
    public func sendFile(at url: URL, named name: String, to peer: Peer) {
        ...
        guard lock.withLock({ activeFileSendName == nil }) else {
        ...
        lock.withLock {
            fileSend = FileSend(data: data, offset: 0, name: name, startedAt: startedAt)
            activeFileSendName = name
        }
        ...
        handler.onDrained = { [weak self] in
            ...
            self.lock.withLock {
                self.streamHandler?.nextChunk = nil
                self.activeFileSendName = nil
            }
    ...
    public func cancelFileSend() {
        let (handler, name) = lock.withLock { () -> (L2CAPStreamHandler?, String?) in
            let name = activeFileSendName
            activeFileSendName = nil
            fileSend = nil
        ...
        // Finish the buffered frame, then append cancel before any subsequent file's header.
        handler?.send(tagged(.fileCancel, Data()))
        continuation.yield(.fileSendFinished(name: name, error: "Canceled"))
    }
...
extension BluetoothTransport: CBPeripheralDelegate {
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        guard let service = peripheral.services?.first(where: { $0.uuid == BLEIdentifiers.serviceUUID }) else {
            continuation.yield(.failure("That camera isn't running Shot Caller."))
            return
        }
